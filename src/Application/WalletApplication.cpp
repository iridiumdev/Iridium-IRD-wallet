...
const char BYTECOIN_URI_SCHEME_NAME[] = "iridium";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("iridiumwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("iridiumwallet.lock"));
...
void WalletApplication::initUi() {
  ...
  if (m_splash != nullptr) {
    m_splash->showMessage(QObject::tr("Initializing GUI..."), Qt::AlignLeft | Qt::AlignBottom, Qt::black);
  }
...
void WalletApplication::newLogString(const QString& _string) {
  ...
  if (match.hasMatch()) {
    ...
    if (m_splash != nullptr) {
      m_splash->showMessage(message, Qt::AlignLeft | Qt::AlignBottom, Qt::black);
    }
  }
}
